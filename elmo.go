...
func main() {
	...
	app.Action = func(cli *cli.Context) error {
		...
		if cli.Bool("use-nagios") {
			...
			fmt.Printf("%v %s %d/%d %v %v\n", time.Now().Format("2006-01-02 15:04:05.000"), cli.String("url"), len(assetsStats), len(assets), gstat.totalResponseTime, gstat.totalResponseSize)
